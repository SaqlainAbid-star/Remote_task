import React, { useState } from 'react';
import { View, Text, FlatList, Image, StyleSheet, TextInput, Modal, TouchableOpacity, ActivityIndicator, Alert, KeyboardAvoidingView, SafeAreaView } from 'react-native';
import { COLORS } from '../assets/constants/Color';
import Ionicons from 'react-native-vector-icons/Ionicons';
import FontAwesome6 from 'react-native-vector-icons/FontAwesome6';

// import DocumentPicker from 'react-native-document-picker';


const TaskDetails = () => {

    const [isModalVisible, setIsModalVisible] = useState(false);
    const [isLoading, setIsLoading] = useState(false);


    const handlePay = () => {
        Alert.alert(
            'Req',
            'Are you sure you want to pay $100?',
            [
                {
                    text: 'Cancel',
                    style: 'cancel',
                },
                {
                    text: 'Send',
                    onPress: () => {
                        setIsModalVisible(true);
                        setIsLoading(true);
                        // Simulating a delay to show loader for 2 seconds
                        setTimeout(() => {
                            setIsLoading(false);
                        }, 1000);
                    },
                },
            ],
            { cancelable: false }
        );
    };

    const toggleModal = () => {
        setIsModalVisible(!isModalVisible);
      };




    return (

        <SafeAreaView style={styles.container}>

            <KeyboardAvoidingView
                style={{ flex: 1 }}
                behavior={Platform.OS == 'ios' ? 'padding' : null}>



                <Text style={styles.title}>Service Providers</Text>

                <View style={styles.userRow}>
                    <Image
                        source={{ uri: "data:image/jpeg;base64," }}
                        style={styles.avatar} />
                    <Text style={styles.text}>Charlie</Text>
                </View>

                <View style={styles.details}>

                    <Text style={styles.text}>Remove 13ft Tree</Text>

                    <View style={styles.time}>
                        <Text style={{ fontSize: 16, marginTop: 2, color: COLORS.liteBlack }}>Date</Text>

                        <View style={{ flexDirection: "row", marginVertical: 5 }}>
                            <Ionicons
                                color={COLORS.black}
                                name="calendar-outline"
                                size={25}
                            />
                            <Text style={styles.timeText}>Fri, 09/16/2022</Text>
                        </View>

                    </View>


                    <View style={styles.time}>
                        <Text style={{ fontSize: 16, marginTop: 2, color: COLORS.liteBlack }}>Time</Text>

                        <View style={{ flexDirection: "row", marginVertical: 5 }}>
                            <Ionicons
                                color={COLORS.black}
                                name="time-outline"
                                size={25}
                            />
                            <Text style={styles.timeText}>3:00PM - 4:00PM</Text>
                        </View>

                    </View>



                </View>



                <View style={styles.details}>

                    <Text style={styles.text}>Amount To Pay</Text>

                    <TextInput
                        style={styles.textInput}
                        placeholder="Type"
                        value='$100'
                    />

                    <Text style={styles.text}>Add Tip</Text>

                    <TextInput
                        style={styles.textInput}
                        placeholder=""
                    />

                    <TouchableOpacity>
                        <Text style={{ marginBottom: 8, color: COLORS.theme }}>Edit Payment Details</Text>
                    </TouchableOpacity>


                </View>


                <View style={{ alignItems: 'center', justifyContent: 'center' }}>

                    <TouchableOpacity style={styles.button} onPress={handlePay}>
                        <Text style={styles.buttonText}>Pay Now</Text>
                    </TouchableOpacity>

                </View>


            </KeyboardAvoidingView >

            <Modal
                animationType="fade"
                transparent={true}
                visible={isModalVisible}
                onRequestClose={() => {
                    setIsModalVisible(!isModalVisible);
                }}
            >
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        {isLoading ? (
                            <>
                                <Text style={{ fontSize: 16, marginBottom: 50, fontWeight: "bold" }}>Payment</Text>
                                <ActivityIndicator size="large" color='#0e293547' />
                                <Text style={{ fontSize: 12, marginTop: 10 }}>Processing</Text>
                            </>
                        ) : (
                            <>
                                <View style={{backgroundColor: "#fef3e6", padding: 8, borderRadius: 8 }}>
                                <FontAwesome6 name="check" size={28} color={COLORS.theme} />
                                </View>

                                <Text style={{ fontSize: 16, margin: 10, color: COLORS.black }}>Payment Successful</Text>

                                <TouchableOpacity style={styles.button} onPress={toggleModal}>
                                    <Text style={styles.buttonText}>Close</Text>
                                </TouchableOpacity>

                            </>
                        )}
                    </View>
                </View>

            </Modal>
        </SafeAreaView>


    );
};

const styles = StyleSheet.create({
    container: {
        flex: 1,
        backgroundColor: '#fff',
        paddingHorizontal: 16,
        paddingTop: 8,
    },
    listContainer: {
        paddingBottom: 16,
    },
    userRow: {
        flexDirection: 'row',
        alignItems: 'center',
        marginTop: 8,
        backgroundColor: '#f6f7fb',
        paddingVertical: 10,
        paddingHorizontal: 10,
        borderRadius: 10,
    },
    details: {
        alignItems: 'flex-start',
        backgroundColor: '#f6f7fb',
        paddingVertical: 10,
        paddingHorizontal: 10,
        borderRadius: 10,
        marginTop: 10,
    },
    time: {
        padding: 10,
        margin: 10,
        backgroundColor: COLORS.white,
        width: "90%",
        height: 80,
        borderRadius: 10
    },
    timeText: {
        marginLeft: 5,
        fontSize: 16,
        marginTop: 2,
        color: COLORS.black
    },
    avatar: {
        width: 60,
        height: 60,
        borderRadius: 30,
        marginRight: 12,
    },
    title: {
        fontSize: 14,
        fontFamily: "Inter-Bold",
        // fontWeight: "bold",
        color: COLORS.black,
    },
    text: {
        fontSize: 16,
        fontFamily: "Inter-Bold",
        color: COLORS.black,
    },
    textInput: {
        height: 40,
        fontSize: 16,
        color: COLORS.green,
        borderColor: 'gray',
        width: "100%",
        borderBottomWidth: 1,
        borderBottomColor: COLORS.liteBlack,
        marginBottom: 5,
    },
    button: {
        marginTop: 15,
        backgroundColor: COLORS.theme,
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'center',
        height: 45,
        borderRadius: 23,
        width: "95%",
    },
    buttonText: {
        color: COLORS.white,
        fontSize: 15
    },
    submit: {
        backgroundColor: COLORS.theme,
        alignItems: 'center',
        justifyContent: 'center',
        height: 50,
        borderRadius: 30,
        width: 100,
        position: 'absolute',
        bottom: 25,
        right: 25,
    },
    modalContainer: {
        flex: 1,
        justifyContent: 'flex-end',
        backgroundColor: 'rgba(0, 0, 0, 0.7)',
    },
    modalContent: {
        backgroundColor: COLORS.white,
        padding: 20,
        borderTopLeftRadius: 20,
        borderTopRightRadius: 20,
        height: 250,
        alignItems: 'center',
        justifyContent: 'center',
    },
    modalButton: {
        padding: 10,
        marginVertical: 5,
        backgroundColor: "#fef3e6",
        borderRadius: 20,
        alignItems: 'center',
        justifyContent: 'center',
    },
});

export default TaskDetails;
